...
int main(int argc, char* argv[])
{
    ...
    cudl::MNIST train_data_loader = cudl::MNIST("D:/github/Learn-CUDA-Programming - Copy/10_deep_learning/01_ann/dataset");
    train_data_loader.train(batch_size_train, true);

    // step 2. model initialization
    cudl::Network model;
    model.add_layer(new cudl::Dense("dense1", 500));
    model.add_layer(new cudl::Activation("relu", CUDNN_ACTIVATION_RELU));
    model.add_layer(new cudl::Dense("dense2", 200));
    model.add_layer(new cudl::Activation("relu", CUDNN_ACTIVATION_RELU));
    model.add_layer(new cudl::Dense("dense3", 100));
    model.add_layer(new cudl::Activation("relu", CUDNN_ACTIVATION_RELU));
    model.add_layer(new cudl::Dense("dense4", 50));
    model.add_layer(new cudl::Activation("relu", CUDNN_ACTIVATION_RELU));
    model.add_layer(new cudl::Dense("dense5", 10));
    ...
    // step 3. train
    int step = 0;
    cudl::Blob<float> *train_data = train_data_loader.get_data();
    cudl::Blob<float> *train_target = train_data_loader.get_target();
    ...
    cudl::MNIST test_data_loader = cudl::MNIST("D:/github/Learn-CUDA-Programming - Copy/10_deep_learning/01_ann/dataset");
